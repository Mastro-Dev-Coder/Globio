    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                <i class="fas fa-clock mr-3 text-red-600"></i>{{ __('ui.latest_videos_page') }}
            </h1>
            <p class="text-gray-600 dark:text-gray-400">
                {{ __('ui.recently_published') }}
            </p>
        </div>
                    ...
                    <a href="{{ route('videos.show', $video) }}" class="group">
                        <div class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700">
                            ...
                            <div class="relative aspect-video bg-gray-200 dark:bg-gray-700 overflow-hidden">
                                ...
                                <!-- New Badge -->
                                @if($video->published_at->diffInHours() < 24)
                                    <div class="absolute top-2 left-2 bg-green-600 text-white text-xs px-2 py-1 rounded flex items-center">
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex items-center space-x-2 mb-2">
                                    @if($video->user->userProfile && $video->user->userProfile->avatar_url)
                                        @php
                                            $avatarUrl = $video->user->userProfile->avatar_url;
                                            $avatarUrl = str_starts_with($avatarUrl, 'http') || str_starts_with($avatarUrl, '/')
                                                ? $avatarUrl
