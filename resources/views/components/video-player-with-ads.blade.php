<div class="artplayer-app relative w-full max-w-screen-xl mx-auto p-4" style="overflow: visible;">
    ...
    <div id="videoWrapper" class="relative w-full aspect-video bg-black group z-10 shadow-2xl rounded-xl overflow-visible"
        ...
        <div class="absolute inset-0 rounded-xl overflow-hidden">
            @if ($video->status !== 'published')
                <div class="absolute inset-0 flex items-center justify-center bg-gray-900 z-20 rounded-xl">
                    <div class="text-center p-8">
                        ...
                        <p class="text-gray-300 mb-6 max-w-md mx-auto">Il video è attualmente in fase di elaborazione e
                            sarà
            ...
            @else
                    ...
                    if (!empty($playlistNextVideos)) {
                        ...
                        foreach ($playlistNextVideos as $idx => $nextV) {
                            $playlistData[] = [
                                ...
                                'title' => $nextV->title,
                                'poster' => $nextV->thumbnail_url ?? '',
                            ];
                        }
                    }

                    $nextVideoUrl = isset($nextVideo) && $nextVideo ? route('videos.show', $nextVideo) : '';
                    $nextVideoPoster = isset($nextVideo) ? $nextVideo->thumbnail_url ?? '' : '';
                ...
                @endphp
                <div id="artplayer" class="w-full h-full rounded-xl" style="overflow: visible;"
                    data-video-src="{{ $video->video_file_url }}" data-poster="{{ $video->thumbnail_url }}"
...
<script type="module">
    document.addEventListener('DOMContentLoaded', () => {
        @if ($video->status === 'published')
            if (window.initArtPlayer) {
                window.initArtPlayer('artplayer');
            }
        @endif
    });
</script>
