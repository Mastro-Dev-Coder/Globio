<div class="group cursor-pointer relative p-1 sm:p-2 rounded-xl w-full max-w-full" data-color-wrapper-playlist data-color-card style="--hover-color: rgba(120, 120, 120, 0.14)">
        ...
        <div class="relative overflow-hidden w-full rounded-xl bg-gray-100 dark:bg-gray-800 aspect-video mb-3 transition-all duration-300"
            ...
            @if ($playlist->videos->count() > 0)
                ...
                @if ($firstVideo->thumbnail_url)
                    <img src="{{ $firstVideo->thumbnail_url }}" alt="{{ $playlist->title }}"
                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" data-color-thumb>
                @else
                    <div
