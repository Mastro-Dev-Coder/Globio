...
class VideoRecommendationService
{
    ...
    private function calculateHistoryScore(Video $video, Collection $watchHistory): float
    {
        ...
        foreach ($watchHistory as $history) {
            ...
            $completionWeight = $this->calculateCompletionWeight($history);
    ...
    public function updateUserInterests(int $userId): void
    {
        ...
        $stats = [
            ...
            'avg_watch_completion' => $watchHistory->avg(function ($item) {
                return $this->calculateCompletionWeight($item) * 100;
            }),
    ...
    public function getRecommendationsFromInterests(int $userId, int $limit = 20, array $filters = []): Collection
    {
        ...
        $scoredVideos = $candidates->map(function ($video) use ($interests) {
            ...
            if (!empty($interests['preferred_categories'])) {
                ...
                $maxCategoryScore = max($interests['preferred_categories']);
                if ($videoCategory && isset($interests['preferred_categories'][$videoCategory]) && $maxCategoryScore > 0) {
                    $score += ($interests['preferred_categories'][$videoCategory] / $maxCategoryScore) * 0.3; // Pesa 30%
                }
            }
    ...
    /**
     * Calcola il completamento in modo sicuro (evita divisioni per zero).
     */
    private function calculateCompletionWeight($history): float
    {
        if ($history->completed) {
            return 1.0;
        }

        $duration = (float) ($history->total_duration ?? 0);
        if ($duration <= 0) {
            return 0.0;
        }

        $watched = max(0.0, (float) ($history->watched_duration ?? 0));

        return min($watched / $duration, 1.0);
    }
}
